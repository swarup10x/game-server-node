...
class RedGreen
{
    ...
    {
        $this->id = $id;
        $this->name = $name == null ? 'red-green_' . date('d-m-y-H:i') : $name;
    ...
    public static function loadCurrentGame()
    {
        try {
            ...
            $row = DB::table('red_green_game')->orderBy('id', 'desc')->take(1)->get()->first();
            $game = Helpers::makeRedGreenFromRow($row);
            return $game;
        } catch (\Exception $e) {
            return null;
        }
    ...
    public static function requestNewGame($startTime)
    {
        ...
        Helpers::saveRedGreenGameToDB($newGame);
        info(' : saved (red-green) to db: ');
        return $newGame;
    }
    ...
    public static function createNewGame($startTime)
    {
        ...
        info(' : creating new game (red-green) : ' . $game->startTime);
        return $game;
    }
    ...
    public static  function cashInBet($request)
    {
        ...
        DB::table('red_green_game_bets')->insert($bet);
    ...
    public static function getUserBet($request)
    {
        ...
        $bet = DB::table('red_green_game_bets')
            ->where('gameId', $gameId)
            ->where('userId', $userId)
            ->get()->first();
        return $bet;
    ...
    public static function rewardAllBets()
    {
        ...
        $bets = DB::table('red_green_game_bets')
