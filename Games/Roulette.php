...
class Roulette
{
    ...
    {
        $this->id = $id;
        $this->name = $name == null ? 'roulette_' . date('d-m-y-H:i') : $name;
    ...
    {
        try {
            $row = DB::table('roulette_game')->orderBy('id', 'desc')->take(1)->get()->first();
            $game = Helpers::makeRouletteFromRow($row);
            return $game;
        } catch (\Exception $e) {
            return null;
        }
    ...
    public static function requestNewGame($startTime)
    {
        ...
        Helpers::saveRouletteGameToDB($newGame);
        info(' : saved (roulette) to db: ');
        return $newGame;
    }
    ...
    public static function createNewGame($startTime)
    {
        ...
        info(' : creating new game (roulette) : ' . $game->startTime);
        return $game;
    }
    ...
    public static  function cashInBet($request)
    {
        ...
        DB::table('roulette_game_bets')->insert($bet);
    ...
    public static function getUserBet($request)
    {
        ...
        $bet = DB::table('roulette_game_bets')
            ->where('gameId', $gameId)
            ->where('userId', $userId)
            ->get()->first();
        return $bet;
    ...
    public static function rewardAllBets()
    {
        ...
        $bets = DB::table('roulette_game_bets')
