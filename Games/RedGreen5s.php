...
class RedGreen5s
{
    ...
    {
        $this->id = $id;
        $this->name = $name == null ? 'red-green5s_' . date('d-m-y-H:i') : $name;
    ...
    public static function loadCurrentGame()
    {
        try {
            ...
            $row = DB::table('red_green5s_game')->orderBy('id', 'desc')->take(1)->get()->first();
            $game = Helpers::makeRedGreen5sFromRow($row);
            return $game;
        } catch (\Exception $e) {
            return null;
        }
    ...
    public static function requestNewGame($startTime)
    {
        ...
        Helpers::saveRedGreen5sGameToDB($newGame);
        info(' : saved (red-green5s) to db: ');
        return $newGame;
    }
    ...
    public static function createNewGame($startTime)
    {
        ...
        info(' : creating new game (red-green5s) : ' . $game->startTime);
        return $game;
    }
    ...
    public static  function cashInBet($request)
    {
        ...
        DB::table('red_green5s_game_bets')->insert($bet);
    ...
    public static function removeBet($request)
    {
        ...
        DB::table('red_green5s_game_bets')
            ->where('userId', $userId)
    ...
    public static function getUserBet($request)
    {
        ...
        $bet = DB::table('red_green5s_game_bets')
            ->where('gameId', $gameId)
            ->where('userId', $userId)
            ->get()->first();
        return $bet;
    ...
    public static function rewardAllBets()
    {
        ...
        $bets = DB::table('red_green5s_game_bets')
