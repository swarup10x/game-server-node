...
class Roulette5s
{
    ...
    {
        $this->id = $id;
        $this->name = $name == null ? 'roulette5s_' . date('d-m-y-H:i') : $name;
    ...
    {
        try {
            $row = DB::table('roulette5s_game')->orderBy('id', 'desc')->take(1)->get()->first();
            $game = Helpers::makeRoulette5sFromRow($row);
            return $game;
        } catch (\Exception $e) {
            return null;
        }
    ...
    public static function requestNewGame($startTime)
    {
        ...
        Helpers::saveRoulette5sGameToDB($newGame);
        info(' : saved (roulette5s) to db: ');
        return $newGame;
    }
    ...
    public static function createNewGame($startTime)
    {
        ...
        info(' : creating new game (roulette5s) : ' . $game->startTime);
        return $game;
    }
    ...
    public static  function cashInBet($request)
    {
        ...
        DB::table('roulette5s_game_bets')->insert($bet);
    ...
    public static function getUserBet($request)
    {
        ...
        $bet = DB::table('roulette5s_game_bets')
            ->where('gameId', $gameId)
            ->where('userId', $userId)
            ->get()->first();
        return $bet;
    ...
    public static function rewardAllBets()
    {
        ...
        $bets = DB::table('roulette5s_game_bets')
