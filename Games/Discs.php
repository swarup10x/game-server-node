...
class Discs
{
    ...
    {
        $this->id = $id;
        $this->name = $name == null ? 'discs_' . date('d-m-y-H:i') : $name;
    ...
    {
        try {
            $row = DB::table('discs_game')->orderBy('id', 'desc')->take(1)->get()->first();
            $discs = Helpers::makeDiscsFromRow($row);
            return $discs;
        } catch (\Exception $e) {
    ...
    public static function requestNewGame($startTime)
    {
        ...
        Helpers::saveDiscsGameToDB($newGame);
        info(' : saved (discs) to db: ');
        return $newGame;
    }
    ...
    public static function createNewGame($startTime)
    {
        ...
        info(' : creating new game (discs) : ' . $game->startTime);
        return $game;
    }
    ...
    public static  function cashInBet($request)
    {
        ...
        DB::table('discs_game_bets')->insert($bet);
