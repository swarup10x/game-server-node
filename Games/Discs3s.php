...
class Discs3s
{
    ...
    {
        $this->id = $id;
        $this->name = $name == null ? 'discs3s_' . date('d-m-y-H:i') : $name;
    ...
    {
        try {
            $row = DB::table('discs3s_game')->orderBy('id', 'desc')->take(1)->get()->first();
            $discs3s = Helpers::makeDiscs3sFromRow($row);
            return $discs3s;
        } catch (\Exception $e) {
    ...
    public static function requestNewGame($startTime)
    {
        ...
        Helpers::saveDiscs3sGameToDB($newGame);
        info(' : saved (discs3s) to db: ');
        return $newGame;
    }
    ...
    public static function createNewGame($startTime)
    {
        ...
        info(' : creating new game (discs3s) : ' . $game->startTime);
        return $game;
    }
    ...
    public static  function cashInBet($request)
    {
        ...
        DB::table('discs3s_game_bets')->insert($bet);
    ...
    public static function getUserBet($request)
    {
        ...
        $bet = DB::table('discs3s_game_bets')
            ->where('gameId', $gameId)
            ->where('userId', $userId)
            ->get()->first();
        return $bet;
    ...
    public static function rewardAllBets()
    {
        ...
        $bets = DB::table('discs3s_game_bets')
